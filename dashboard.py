...
st.set_page_config(page_title="Tweets Dashboard", layout="wide")
...
def selectAuthor():
    df = loadData()
    hashTags = st.multiselect("choose Author", list(df['original_author'].unique()))
    if hashTags:
...
def selectPlace():
    df = loadData()
    hashTags = st.multiselect("choose Place", list(df['place'].unique()))
    if hashTags:
        df = df[np.isin(df, hashTags).any(axis=1)]
        st.write(df)
...
st.title("China and Taiwan Conflict")
